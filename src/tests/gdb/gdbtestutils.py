...
def register_stop_handler(scenario_module):
    """Register a one-shot stop handler that only runs the scenario when
    we stop at the JITted bpmd breakpoint. It does NOT issue any continues.
    Use from the harness after a CLI 'continue'.
    """
    def _trace(msg: str):
        try:
            gdb.write(f"[trace] {msg}\n")
    ...
    def _on_stop(event):
        ...
        try:
            ...
            pass
        if pc is None or not _pc_matches_cli_bp(pc):
            _trace('stop ignored (not at bpmd jit bp)')
            return
